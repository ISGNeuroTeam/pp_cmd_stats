...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"stats_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f stats-*.tar.gz
	echo Create archive \"stats-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../stats-$(VERSION)-$(BRANCH).tar.gz stats
...
stats.tar.gz: build
	cd make_build; tar czf ../stats.tar.gz stats && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./stats make_build
	cp *.md make_build/stats/
